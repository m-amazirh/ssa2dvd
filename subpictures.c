	...
	for (frame = images; frame != NULL; frame = frame->next){
		...
		for (y = 0; y < frame->h; y++) {
			for (x = 0; x < frame->w; x++) {
				...
				unsigned char src_alpha_prenormalized = frame->bitmap[y * frame->stride + x];
				unsigned src_alpha = (src_alpha_prenormalized) * opacity / 255 ;
				
				int dst_old_color = gdImageTrueColorPixel(s->canvas, dst_x, dst_y);
				...
				if( ((dst_y * s->canvas_w * 4) + dst_x * 4) > (s->canvas_w * s->canvas_h * 4) )
					continue;
				
				int dst_new_color,dst_new_red,dst_new_green,dst_new_blue,dst_new_alpha;
				dst_new_blue = (src_alpha * b + (255 - src_alpha) * dst_old_blue) / 255;
				dst_new_green = (src_alpha * g + (255 - src_alpha) * dst_old_green ) / 255;
				dst_new_red = (src_alpha * r + (255 - src_alpha) * dst_old_red) / 255;
                
                dst_new_color = gdImageColorAllocate(s->canvas, dst_new_red ,dst_new_green, dst_new_blue);
...
char* long2time ( long long l )
{
	...
	sprintf(time,"%02lli:%02lli:%02lli.%02lli",hours,minutes%60,seconds%60,millis/10);
