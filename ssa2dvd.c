...
{
    printf("\nUsage : \n");
    printf("\t%s -w width -h height -o output_directory -s subtitle_file [-r right_margin] [-l left_margin] \n",program_name);
}
...
settings process_arguments(char **argv, char argc)
{
    ...
    for (i=1; i + 1 < argc; i += 2){
        ...
        switch(option[1])
        {
            ...
            default : printf("%s isn't a valid option\n");
                      usage(argv[0]);
                      exit(1);
    ...
    if (s.subpicture_width == -1)   { printf("Width parameter required ( -w )\n"); usage(argv[0]); exit(1); }
    if (s.subpicture_height == -1)  { printf("Height parameter required ( -h )\n"); usage(argv[0]); exit(1); }
    if (s.subtitle_path == NULL)    { printf("A subtitle file is required ( -s )\n"); usage(argv[0]); exit(1); }
    if (s.output_directory == NULL) { printf("A path to an output directory is required ( -o )\n"); usage(argv[0]); exit(1); }
...
ASS_Renderer* init_ass_renderer(ASS_Library *library, settings s)
{
    ...
    ass_set_margins(renderer, s.top_margin, s.bottom_margin, s.left_margin, s.right_margin);
...
int main(int argc,char **argv)
{
    ...
    xml_path = (char *) calloc(strlen(st.output_directory) + strlen(st.name) + 1 + 4 + 1, sizeof(char));
    ...
    for (pos = 0; pos <= pos_max; pos += 25)
    {
        ...
        if (detect_change != 0)
        {
            if (current_subpicture != NULL)
            {
                ...
                image_path = (char *) calloc(strlen(st.output_directory) + strlen(st.name) + 10, sizeof(char));
                image_filename = (char *) calloc(strlen(st.name) + 10, sizeof(char));

                sprintf(image_filename,"%s_%i.png",st.name,current_subpicture->id);
                sprintf(image_path,"%s/%s",st.output_directory,image_filename);
                ...
                subpicture_append_info_to_xml(current_subpicture, image_filename, xml_out);
            ...
            if(images != NULL)
            {
                ...
                subpicture_init(current_subpicture, ++id, st.subpicture_width, st.subpicture_height, pos);
